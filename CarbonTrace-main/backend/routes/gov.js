...
// Memory storage — we upload buffer directly to Pinata (no disk writes)
const upload = multer({
...
// ─── Land Requests List ─────────────────────────────────────────────────────────
router.get('/land-requests', authenticate, authorize('GOVERNMENT', 'PANCHAYAT'), async (req, res) => {
  try {
    const db = getModels();
    if (!db) return res.json([]);
...
// ─── Get Single Land Request Details ──────────────────────────────────────────
router.get('/land-requests/:id', authenticate, async (req, res) => {
  try {
    const db = getModels();
    ...
    const request = await db.LandRequest.findByPk(req.params.id, {
      include: [
        ...
        { model: db.RegisteredLand, as: 'registeredLand' },
      ],
    });
    if (!request) return res.status(404).json({ message: 'Land request not found' });
    res.json(request);
  } catch (err) {
...
// ─── Upload documents for land request to Pinata IPFS (Panchayat/Farmer) ─────
// Accepts: satbara (required) + other_doc (optional) as multipart/form-data files
router.post(
  '/land-requests/:id/upload-document',
  authenticate,
  upload.fields([{ name: 'satbara', maxCount: 1 }, { name: 'other_doc', maxCount: 1 }]),
  async (req, res) => {
    const db = getModels();
    if (!db) return res.status(503).json({ message: 'DB unavailable' });
    try {
      ...
      const satbaraFile = req.files?.satbara?.[0];
      const otherDocFile = req.files?.other_doc?.[0];

      if (!satbaraFile && !otherDocFile) {
        return res.status(400).json({ message: 'Please upload at least one document (satbara or other_doc)' });
      }

      let satbaraCid = null;
      let otherDocCid = null;

      // Upload satbara to Pinata IPFS
      if (satbaraFile) {
        console.log(`[PINATA] Uploading satbara for REQ-${request.id}: ${satbaraFile.originalname}`);
        const result = await pinFileToIPFS(
          satbaraFile.buffer,
          satbaraFile.originalname,
          `Satbara-REQ-${request.id}-${request.owner_name}`
        );
        if (result.success) {
          satbaraCid = result.cid;
          console.log(`[PINATA] ✅ Satbara pinned: ${satbaraCid}`);
        } else {
          console.error('[PINATA] Satbara upload failed:', result.error);
          return res.status(500).json({ message: `Satbara upload to IPFS failed: ${result.error}` });
        }
      }

      // Upload other doc to Pinata IPFS
      if (otherDocFile) {
        console.log(`[PINATA] Uploading other_doc for REQ-${request.id}: ${otherDocFile.originalname}`);
        const result = await pinFileToIPFS(
          otherDocFile.buffer,
          otherDocFile.originalname,
          `OtherDoc-REQ-${request.id}-${request.owner_name}`
        );
        if (result.success) {
          otherDocCid = result.cid;
          console.log(`[PINATA] ✅ OtherDoc pinned: ${otherDocCid}`);
        } else {
          console.error('[PINATA] OtherDoc upload failed:', result.error);
          // Non-fatal for other_doc
        }
      }

      if (db.sequelize?.options?.dialect === 'postgres') {
        await db.sequelize.query(
          `SELECT setval(pg_get_serial_sequence('land_documents', 'id'), COALESCE((SELECT MAX(id) FROM "land_documents"), 1));`
        ).catch(() => {});
      }

      // Check if a document record already exists (update) or create new one
      const existingDoc = await db.LandDocument.findOne({ where: { land_request_id: request.id } });

      let doc;
      if (existingDoc) {
        const updates = {};
        if (satbaraCid) updates.satbara_cid = satbaraCid;
        if (otherDocCid) updates.other_docs_cid = otherDocCid;
        updates.doc_status = 'VERIFIED';
        updates.uploaded_at = new Date();
        await existingDoc.update(updates);
        doc = existingDoc;
      } else {
        doc = await db.LandDocument.create({
          land_request_id: request.id,
          satbara_cid: satbaraCid || null,
          other_docs_cid: otherDocCid || null,
          doc_status: 'VERIFIED',
          uploaded_at: new Date(),
        });
      }

      console.log(`[GOV] ✅ Documents saved — satbara: ${satbaraCid}, other: ${otherDocCid}`);
      res.json({
        success: true,
        satbara_cid: satbaraCid,
        other_docs_cid: otherDocCid,
        satbara_url: satbaraCid ? `https://gateway.pinata.cloud/ipfs/${satbaraCid}` : null,
        other_doc_url: otherDocCid ? `https://gateway.pinata.cloud/ipfs/${otherDocCid}` : null,
        doc,
      });
    } catch (err) {
...
// ─── Get farmer-uploaded documents for a land request (Government view) ────────
router.get('/land-requests/:id/farmer-documents', authenticate, authorize('GOVERNMENT'), async (req, res) => {
  const db = getModels();
  if (!db) return res.status(503).json({ message: 'DB unavailable' });
  try {
    const request = await db.LandRequest.findByPk(req.params.id, {
    ...
    const doc = request.documents?.[0] || null;
    res.json({
      request_id: request.id,
      owner_name: request.owner_name,
      panchayat: request.panchayat,
      documents: doc ? {
        satbara_cid: doc.satbara_cid,
        other_docs_cid: doc.other_docs_cid,
        satbara_url: doc.satbara_cid ? `https://gateway.pinata.cloud/ipfs/${doc.satbara_cid}` : null,
        other_doc_url: doc.other_docs_cid ? `https://gateway.pinata.cloud/ipfs/${doc.other_docs_cid}` : null,
        doc_status: doc.doc_status,
        uploaded_at: doc.uploaded_at,
      } : null,
      has_documents: Boolean(doc?.satbara_cid || doc?.other_docs_cid),
    });
  } catch (err) {
    console.error('[FARMER_DOCS]', err.message);
    res.status(500).json({ message: err.message });
  }
});
...
router.post('/land-requests/:id/prepare-approval', authenticate, authorize('GOVERNMENT'), async (req, res) => {
  ...
  try {
    ...
    const count = Math.max(await db.RegisteredLand.count(), maxId);
    const uniqueSuffix = Date.now().toString().slice(-3);
    const landIdGov = `GOV-${year}-${state}-${String(count + 1).padStart(4, '0')}-${uniqueSuffix}`;
...
router.patch('/land-requests/:id/approve-with-hash', authenticate, authorize('GOVERNMENT'), async (req, res) => {
  ...
  try {
    ...
    if (db.sequelize?.options?.dialect === 'postgres') {
      await db.sequelize.query(
        `SELECT setval(pg_get_serial_sequence('registered_lands', 'id'), COALESCE((SELECT MAX(id) FROM "registered_lands"), 1));`
      ).catch(() => {});
    }
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      // Ensure PostgreSQL sequence is synced above max(id)
      if (db.sequelize?.options?.dialect === 'postgres') {
        await db.sequelize.query(
          `SELECT setval(pg_get_serial_sequence('land_requests', 'id'), COALESCE((SELECT MAX(id) FROM "land_requests"), 1));`
        ).catch(() => {});
      }
      ...
      console.error('[PANCHAYAT_SUBMIT]', err.name, err.message, err.errors || '');
      if (err.name === 'SequelizeValidationError' || err.name === 'SequelizeUniqueConstraintError') {
        const msgs = err.errors?.map(e => `${e.path}: ${e.message}`)?.join(', ') || err.message;
        return res.status(400).json({ message: msgs });
      }
      if (err.name === 'SequelizeForeignKeyConstraintError') {
        return res.status(400).json({ message: 'Invalid Panchayat user ID' });
      }
      res.status(500).json({ message: err.message });
    }
  }
