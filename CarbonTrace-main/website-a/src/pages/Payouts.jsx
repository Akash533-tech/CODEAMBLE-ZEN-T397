...
const STATUS_MAP = {
  COMPLETED: { bg: 'gl-badge-approved', icon: CheckCircle2 },
  PROCESSING: { bg: 'gl-badge-info', icon: Clock },
  PENDING: { bg: 'gl-badge-pending', icon: Clock },
  FAILED: { bg: 'gl-badge-rejected', icon: XCircle },
};
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-ct-light-text">NGO / Panchayat Payouts</h1>
          <p className="text-ct-light-muted text-sm mt-0.5">Quality-based payout tracking via Razorpay</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="gl-card p-4">
          <div className="flex items-center gap-2 mb-2">
            <IndianRupee size={14} className="text-ct-primary" />
            <p className="text-xs text-ct-light-muted uppercase font-semibold">Total Paid</p>
          </div>
          <p className="text-2xl font-bold text-ct-light-text">{fmtINR(totalPaid)}</p>
        </div>
        <div className="gl-card p-4">
          <div className="flex items-center gap-2 mb-2">
            <Clock size={14} className="text-amber-600" />
            <p className="text-xs text-ct-light-muted uppercase font-semibold">Pending / Processing</p>
          </div>
          <p className="text-2xl font-bold text-ct-light-text">{fmtINR(totalPending)}</p>
        </div>
        <div className="gl-card p-4">
          <div className="flex items-center gap-2 mb-2">
            <Building2 size={14} className="text-ct-primary" />
            <p className="text-xs text-ct-light-muted uppercase font-semibold">Active NGOs</p>
          </div>
          <p className="text-2xl font-bold text-ct-light-text">{new Set(MOCK_PAYOUTS.map((p) => p.ngo)).size}</p>
        </div>
      </div>
        ...
        <table className="gl-table">
          ...
          <tbody>
            {MOCK_PAYOUTS.map((p) => {
              ...
              return (
                <tr key={p.id}>
                  <td className="font-mono text-xs text-ct-primary font-semibold">PAY-{String(p.id).padStart(4, '0')}</td>
                  <td className="font-medium text-ct-light-text">{p.ngo}</td>
                  <td className="text-ct-light-muted text-xs">{p.panchayat}</td>
                  <td className="font-mono text-xs text-ct-light-muted">{p.landId}</td>
                  <td className="font-mono font-semibold">₹{p.amount.toLocaleString('en-IN')}</td>
                  <td>
                    <span className={`text-xs font-semibold ${p.quality >= 90 ? 'text-emerald-600' : p.quality >= 80 ? 'text-amber-600' : 'text-red-600'}`}>
                      {p.quality}%
                    </span>
                  </td>
                  <td>
