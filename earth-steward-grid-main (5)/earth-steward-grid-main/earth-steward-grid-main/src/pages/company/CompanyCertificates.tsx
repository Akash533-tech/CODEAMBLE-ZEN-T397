...
import type { NFTMintResult } from '@/services/company';
...
import {
  ...
  Calendar, Building2, Leaf, Award, Globe, Printer, Wallet, ExternalLink, CheckCircle2, XCircle, Sparkles
} from 'lucide-react';
...
export default function CompanyCertificates() {
  ...
  // NFT minting state
  const [mintDialogCert, setMintDialogCert] = useState<any | null>(null);
  // Track in-memory NFT status for certs that were just minted this session
  const [sessionMintResults, setSessionMintResults] = useState<Record<string, NFTMintResult>>({});

  const queryClient = useQueryClient();
  const { data, isLoading } = useQuery({ queryKey: ['company-certs'], queryFn: getCertificates });
  const { companyUser } = useAuthStore();

  const handleMintSuccess = (result: NFTMintResult) => {
    // Store result in session so UI updates immediately without refetch
    setSessionMintResults(prev => ({ ...prev, [result.certificate_id]: result }));
    // Also invalidate query so next open shows persisted state
    queryClient.invalidateQueries({ queryKey: ['company-certs'] });
    setMintDialogCert(null);
  };
  ...
  return (
    <div className="space-y-6">
            ...
            >
              <CardContent className="p-0">
                ...
                <div className="p-4 space-y-3">
                  ...
                  {/* NFT Status: already minted */}
                  {(() => {
                    const sessionResult = sessionMintResults[cert.certificate_id];
                    const isMinted = cert.nft_status === 'minted' || !!sessionResult;
                    const txHash = sessionResult?.nft_tx_hash || cert.nft_tx_hash;
                    const tokenId = sessionResult?.nft_token_id || cert.nft_token_id;
                    const contractAddr = sessionResult?.nft_contract_address || cert.nft_contract_address;
                    if (isMinted && txHash) {
                      return (
                        <div className="space-y-1">
                          <div className="flex items-center gap-1">
                            <Badge className="bg-purple-600 text-white text-[9px] gap-1 px-1.5 py-0.5">
                              <Sparkles className="h-2.5 w-2.5" /> NFT Minted ✓
                            </Badge>
                            <span className="text-[9px] text-muted-foreground">Token #{tokenId}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <a href={`https://sepolia.etherscan.io/tx/${txHash}`} target="_blank" rel="noopener noreferrer"
                              className="flex items-center gap-1 text-[10px] text-gray-600 hover:text-gray-800"
                              onClick={(e) => e.stopPropagation()}>
                              <ExternalLink className="h-2.5 w-2.5" /> Etherscan
                            </a>
                            <span className="text-muted-foreground text-[10px]">·</span>
                            <a href={`https://testnets.opensea.io/assets/sepolia/${contractAddr}/${tokenId}`} target="_blank" rel="noopener noreferrer"
                              className="flex items-center gap-1 text-[10px] text-blue-600 hover:text-blue-800"
                              onClick={(e) => e.stopPropagation()}>
                              <ExternalLink className="h-2.5 w-2.5" /> OpenSea
                            </a>
                          </div>
                        </div>
                      );
                    }
                    // Blockchain TX link (old-style, non-NFT)
                    if (cert.blockchain_tx_hash && cert.blockchain_tx_hash.startsWith('0x')) {
                      return (
                        <a href={`https://sepolia.etherscan.io/tx/${cert.blockchain_tx_hash}`}
                  ...
                  {/* Mint as NFT — only if not already minted */}
                  {cert.nft_status !== 'minted' && !sessionMintResults[cert.certificate_id] && (
                    <Button
                      size="sm"
                      variant="outline"
                      className="w-full text-xs mt-1 border-purple-300 text-purple-700 hover:bg-purple-50 gap-1"
                      onClick={(e) => { e.stopPropagation(); setMintDialogCert(cert); }}
                    >
                      <Sparkles className="h-3.5 w-3.5" />
                      {cert.nft_status === 'failed' ? 'Retry Mint as NFT' : 'Mint as NFT'}
                    </Button>
                  )}
                </div>
              </CardContent>
      ...
      {mintDialogCert && (
        <MintNFTDialog
          open={!!mintDialogCert}
          onOpenChange={(open) => { if (!open) setMintDialogCert(null); }}
          cert={mintDialogCert}
